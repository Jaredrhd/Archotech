...
require_once($CFG->dirroot.'/question/type/logicgate/shapes.php');

define('QTYPE_LOGICGATE_ALLOWED_TAGS_IN_MARKER', '<br><i><em><b><strong><sup><sub><u><span>');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_logicgate_edit_form extends qtype_ddtoimage_edit_form_base {
    ...
    protected function definition_inner($mform) {
        $mform->addElement('advcheckbox', 'showmisplaced', ' ',
                                                get_string('showmisplaced', 'qtype_logicgate'));
        parent::definition_inner($mform);

        $mform->addHelpButton('drops[0]', 'dropzones', 'qtype_logicgate');
    }

    public function js_call() {
        global $PAGE;
        $maxsize = ['width' => QTYPE_LOGICGATE_BGIMAGE_MAXWIDTH,
            'height' => QTYPE_LOGICGATE_BGIMAGE_MAXHEIGHT];

        $PAGE->requires->js_call_amd('qtype_logicgate/form', 'init', [$maxsize]);
    }


    protected function definition_draggable_items($mform, $itemrepeatsatstart) {
        $mform->addElement('header', 'draggableitemheader',
                                get_string('markers', 'qtype_logicgate'));
        ...
        $this->repeat_elements($this->draggable_item($mform), $itemrepeatsatstart,
                ...
                get_string('addmoreitems', 'qtype_logicgate'), true);
    }
    ...
    protected function draggable_item($mform) {
        ...
        $noofdragoptions = array(0 => get_string('infinite', 'qtype_logicgate'));
        foreach (range(1, 6) as $option) {
            $noofdragoptions[$option] = $option;
        }
        $grouparray[] = $mform->createElement('select', 'noofdrags', get_string('noofdrags', 'qtype_logicgate'), $noofdragoptions);

        $draggableimageitem[] = $mform->createElement('group', 'drags',
                                            get_string('marker_n', 'qtype_logicgate'), $grouparray);
    ...
    protected function drop_zone($mform, $imagerepeats) {
        $grouparray = array();
        $shapearray = qtype_logicgate_shape::shape_options();
        $grouparray[] = $mform->createElement('select', 'shape',
                                    get_string('shape', 'qtype_logicgate'), $shapearray);
        $markernos = array();
        $markernos[0] = '';
    ...
    protected function get_hint_fields($withclearwrong = false, $withshownumpartscorrect = false) {
        ...
        $repeated[] = $mform->createElement('checkbox', 'hintoptions',
                        '',
                        get_string('stateincorrectlyplaced', 'qtype_logicgate'));
        $repeated[] = $mform->createElement('checkbox', 'hintclearwrong',
                        '',
                        get_string('clearwrongparts', 'qtype_logicgate'));
    ...
    public function data_preprocessing($question) {
        ...
        file_prepare_draft_area($draftitemid, $this->context->id, 'qtype_logicgate',
    ...
    public function validation($data, $files) {
        ...
        if ($bgimagesize === null) {
            $errors["bgimage"] = get_string('formerror_nobgimage', 'qtype_logicgate');
        }
        ...
        for ($i = 0; $i < $data['nodropzone']; $i++) {
            ...
            if ($choicepresent) {
                ...
                if ($bgimagesize !== null) {
                    ...
                    $shapeobj = qtype_logicgate_shape::create($shape, $coordsstring);
                    ...
                    if ($interpretererror !== false) {
                        ...
                        $errors["drops[{$i}]"] =
                                            get_string('formerror_'.$errorcode, 'qtype_logicgate');
                    }
                }
            } else {
                if (trim($data['drops'][$i]['coords']) !== '') {
                    $errorcode = 'noitemselected';
                    $errors["drops[{$i}]"] = get_string('formerror_'.$errorcode, 'qtype_logicgate');
                }
            }
        ...
        for ($dragindex = 0; $dragindex < $data['noitems']; $dragindex++) {
            ...
            if ($label != strip_tags($label, QTYPE_LOGICGATE_ALLOWED_TAGS_IN_MARKER)) {
                $errors["drags[{$dragindex}]"]
                    = get_string('formerror_onlysometagsallowed', 'qtype_logicgate',
                                  s(QTYPE_LOGICGATE_ALLOWED_TAGS_IN_MARKER));
            }
        }
        return $errors;
    }
