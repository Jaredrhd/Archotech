 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class qtype_logicgate_shape {
    /** @var bool Indicates if there is an error */
    protected $error = false;

    /** @var string The shape class prefix */
    protected static $classnameprefix = 'qtype_logicgate_shape_';
    ...
    public function get_coords_interpreter_error() {
        if ($this->error) {
            ...
            return get_string('formerror_'.$this->error, 'qtype_logicgate', $a);
        } else {
            return false;
        }
    }
    ...
    public static function human_readable_name($lowercase = false) {
        ...
        return get_string($stringid, 'qtype_logicgate');
    }

    public static function human_readable_coords_format() {
        return get_string('shape_'.self::name().'_coords', 'qtype_logicgate');
    }
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_logicgate_shape_rectangle extends qtype_logicgate_shape {
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_logicgate_shape_circle extends qtype_logicgate_shape {
    /** @var int X center */
    protected $xcentre;
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_logicgate_shape_polygon extends qtype_logicgate_shape {
    /**
    ...
    public function is_point_in_shape($xy) {
        ...
        $point = new qtype_logicgate_point($xy[0], $xy[1]);
        $windingnumber = 0;
        foreach ($this->coords as $index => $coord) {
            $start = new qtype_logicgate_point($this->coords[$index][0], $this->coords[$index][1]);
            if ($index < count($this->coords) - 1) {
                $endindex = $index + 1;
            } else {
                $endindex = 0;
            }
            $end = new qtype_logicgate_point($this->coords[$endindex][0], $this->coords[$endindex][1]);
     ...
     *
     * @param qtype_logicgate_point $start first of two points on the infinite line.
     * @param qtype_logicgate_point $end second of two points on the infinite line.
     * @param qtype_logicgate_point $point the oint to test.
     * @return number > 0 if the point is left of the line.
     *                 = 0 if the point is on the line.
     *                 < 0 if the point is right of the line.
     */
    protected function is_left(qtype_logicgate_point $start, qtype_logicgate_point $end,
            qtype_logicgate_point $point) {
